...
const RolesDashboard: React.FC = () => {
  ...
  const [selectedDeleteRoleId, setSelectedDeleteRoleId] = useState<number>(0);
  ...
  return (
    <View style={styles.container}>
      ...
      <Modal title={t('title.confirm.delete')} visible={deleteModal} onClose={cancelDeleteRole} size="xxs" position="center">
        <Text style={{ marginBottom: 20, alignSelf: 'center', color: colors.text }}> {t('confirm.delete.message.role')} </Text>
        <View style={{ flexDirection: 'column', alignItems: 'center' }}>
          ...
          <Button title={t('button.delete')} size="xxs" color={colors.whiteText} style={[styles.confirmButton, {backgroundColor: colors.darksteel}]}
            onPress={() => confirmDeleteRole(selectedDeleteRoleId)} 
          />
        </View>
      </Modal>
    </View>

...
const ProjectsTable: React.FC<ProjectTableProps> = ({ projects, onDelete, onEdit, setShowModalForm }) => {
  ...
  (
    <View style={{ alignItems: 'flex-start' }}>
      <Button title={t("button.add.project")} onPress={() => setShowModalForm(true)} height={50} 
        color='white' style ={{backgroundColor: colors.darksteel, borderRadius: 6}} width={windowWidth*0.1}/>
    </View>
  );

  const columns: { header: string; accessor?: keyof Project; filterable?: boolean, sortable?: boolean,  width?: number; 
  ...
  return (
    <Table columns={columns} data={projects} minRowHeight={50} renderHeaderButton={renderHeaderButton} paginationEnabled/>
  );
};
